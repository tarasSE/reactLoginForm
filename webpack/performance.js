...
fs.readFile('./build/index.html', 'utf8', (err, html) => {
    ...
    const css = [...dom.window.document.querySelectorAll('link')].filter(l => l.href === 'style.css')[0];
    const js = [...dom.window.document.querySelectorAll('script')].filter(s => s.src === 'bundle.js')[0];
